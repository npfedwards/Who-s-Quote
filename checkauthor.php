<?php
	...
	//Get user from table
	$query="SELECT * FROM ".$tableprefix."users WHERE UserString='$user' AND IP='$ip'";
	...
	if(mysql_num_rows($result)==1){ //if there is a user
		...
		$query="SELECT * FROM ".$tableprefix."users WHERE UserString='$user'";
		$result=mysql_query($query) or die(mysql_error());
		...
		$query="INSERT INTO ".$tableprefix."users (UserString, IP) VALUES ('$user','$ip')";
		mysql_query($query) or die(mysql_error());
		$userid=mysql_insert_id();
	}
	
	//Does the Author name match the DB?
	$query="SELECT * FROM ".$tableprefix."quotes LEFT JOIN ".$tableprefix."authors ON ".$tableprefix."quotes.AuthorID=".$tableprefix."authors.AuthorID WHERE QuoteID='$q' AND ".$tableprefix."authors.AuthorID='$author' LIMIT 0,1";
	$result=mysql_query($query) or die(mysql_error());
	if(mysql_num_rows($result)==1){
		//Database insert for correctness
		$query="INSERT INTO ".$tableprefix."answers (UserID, QuoteID, Timestamp, Correct) VALUES ('$userid', '$q', '".time()."', '1')";
		mysql_query($query) or die(mysql_error());
		
		$query="SELECT * FROM ".$tableprefix."answers WHERE UserID='$userid'";
		$result=mysql_query($query) or die(mysql_error());
		$quotes=mysql_num_rows($result);
		$query="SELECT * FROM ".$tableprefix."answers WHERE UserID='$userid' AND Correct='1'";
		$result=mysql_query($query) or die(mysql_error());
		...
		$query="INSERT INTO ".$tableprefix."answers (UserID, QuoteID, Timestamp, Correct) VALUES ('$userid', '$q', '".time()."', '0')";
		mysql_query($query) or die(mysql_error());
